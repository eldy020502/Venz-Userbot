...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"🔥ᴘᴇꜱᴀɴ ᴏᴛᴏᴍᴀᴛɪꜱ\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} ᴀꜰᴋ ᴅᴜʟᴜ\n┣ ᴀʟᴀꜱᴀɴ : {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
    else:
        await afk_e.edit("🔥 ᴀ ꜰ ᴋ\n╭╼═══╾╼═══╾╼═══╾ \n┣ ᴊᴀɴɢᴀɴ ᴅɪᴄᴀʀɪ ✨\n╰╼═══╾╼═══╾")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("🔥 ᴀ ꜰ ᴋ"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("🔥ᴏɴʟɪɴᴇ")
        time.sleep(3)
        await msg.delete()
        ...
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"🔥ᴘᴇꜱᴀɴ ᴏᴛᴏᴍᴀᴛɪꜱ\n╭╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼═══╾")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"🔥ᴘᴇꜱᴀɴ ᴏᴛᴏᴍᴀᴛɪꜱ\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
                else:
