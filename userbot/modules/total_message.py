...
async def _(event):
    ...
    if k:
        ...
        return await event.edit(f"Total Pesan Dari Si Cantik/Ganteng {u}. Total Chats `{a.total}`")
    u = event.pattern_match.group(1)
    if not u:
        u = "me"
    a = await bot.get_messages(event.chat_id, 0, from_user=u)
    await event.edit(f"Total Pesan Dari Si Cantik/Ganteng {u}. Total Chats `{a.total}`")
