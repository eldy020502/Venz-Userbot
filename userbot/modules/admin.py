...
@venz_cmd(pattern="promote(?:\\s|$)([\\s\\S]*)")
...
async def promote(event):
    ...
    eventvenz = await edit_or_reply(event, "`Promosikan Pengguna Sebagai Admin... Mohon Menunggu`")
    ...
    try:
        ...
        await edit_delete(eventvenz, "`Berhasil Menambahkan Pengguna Ini Sebagai Admin!`")

    # If Telethon spit BadRequestError, assume
    # we don't have Promote permission
    except BadRequestError:
        return await eventvenz.edit(NO_PERM)
...
async def demote(event):
    ...
    eventvenz = await edit_or_reply(event, "`Sedang Melepas Admin...`")
    ...
    # Assume we don't have permission to demote
        ...
        return await eventvenz.edit(NO_PERM)
    await edit_delete(eventvenz, "`Berhasil Melepas Pengguna Ini Sebagai Admin!`")
...
async def ban(bon):
    ...
    venz = await edit_or_reply(bon, "`Melakukan Banned!`")
...
async def nothanos(unbon):
    ...
    venz = await edit_or_reply(unbon, "`Sedang Melakukan Unban...`")
    ...
    try:
        ...
        await edit_delete(venz, "```Berhasil Melepas Ban Pengguna!```")
        ...
        await edit_delete(venz, "`Sepertinya Terjadi Kesalahan!`")


@venz_cmd(pattern="mute(?: |$)(.*)")
...
async def spider(spdr):
    ...
    venz = await edit_or_reply(spdr, "`Sedang melakukan Mute...`")
    ...
    if user.id == self_user.id:
        return await edit_or_reply(venz,
                                   "`Tangan Terlalu Pendek, Tidak Bisa Membisukan Diri Sendiri...\n(ヘ･_･)ヘ┳━┳`"
                                   )

    # If everything goes well, do announcing and mute
    await venz.edit("`Telah Dibisukan!`")
    if mute(spdr.chat_id, user.id) is False:
        return await venz.edit("`Pengguna Sudah Dibisukan!`")
    else:
        try:
            ...
            if reason:
                await venz.edit(f"**Pengguna Telah Dibisukan!**\n**Alasan:** `{reason}`")
            else:
                await venz.edit("`Telah Dibisukan!`")
            ...
            return await edit_delete(venz, "`Terjadi Kesalahan!`")


@venz_cmd(pattern="unmute(?: |$)(.*)")
...
async def unmoot(unmot):
    ...
    venz = await edit_or_reply(unmot, "```Melakukan Unmute...```")
    ...
    if unmute(unmot.chat_id, user.id) is False:
        ...
        try:
            ...
            await edit_delete(venz, "```Berhasil Melakukan Unmute! Pengguna Sudah Tidak Dibisukan```")
        except UserIdInvalidError:
            return await edit_delete(venz, "`Terjadi Kesalahan!`")
...
async def ungmoot(un_gmute):
    ...
    venz = await edit_or_reply(un_gmute, "`Sedang melakukan membuka Global Mute...`")
    ...
    await venz.edit("```Membuka Global Mute Pengguna...```")

    if ungmute(user.id) is False:
        await venz.edit("`Pengguna Sedang Tidak Di Gmute!`")
    else:
...
async def gspider(gspdr):
    ...
    venz = await edit_or_reply(gspdr, "`Processing...`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return

    # If pass, inform and start gmuting
    await venz.edit("`Pengguna Berhasil Dibisukan!`")
    if gmute(user.id) is False:
        await edit_delete(gspdr, "`Kesalahan! Pengguna Sudah Dibisukan.`")
    else:
        if reason:
            await venz.edit(f"**Dibisukan Secara Global!**\n**Alasan:** `{reason}`")
        else:
